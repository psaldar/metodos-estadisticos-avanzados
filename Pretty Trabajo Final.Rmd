...
$$ CostoDeVentasDif(t) = \dfrac{CostoDeVentas(t) – CostoDeVentas(t-1)}{CostoDeVentas(t)} $$
...
Para fines de intepretación de los modelos creados en el trabajo, consideraremos la transformación logarítmica tanto para las los predictores como para la variable respuesta, así la transformación que se realiza es:

$$T(x) = log(1+x) $$
Por ejemplo, la variable de salida tendra la forma

$$ T(CostoDeVentasDif) = log(\dfrac{CostoDeVentas(t) – CostoDeVentas(t-1)}{CostoDeVentas(t)})$$
...
Cabe mencionar que al momento de realizar las interpretaciones de los resultados del modelo, se regresa la variable a su representación de niveles
...
Recordar que tal como se mencionó al inicio, se realizó una transformación logarítmica tanto a las variables macroeconómicas, como a la variable respuesta (diferencia porcentual de los costos de venta), por lo que al momento de la interpretación de los modelos, estaremos trabajando con regresiones del estilo log-log
...
La selección de ambos conjuntos se basa en la información de las empresas, pues se desea que el costo promedio de ventas las empresas que se considera en el conjunto de entrenamiento, sea similar al costo promedio de ventas de las empresas en el conjunto de prueba. Partiendo de ese objetivo para la partición, se consideran 100.000 combinaciones diferentes de las empresas (combinaciones donde el conjunto de entrenamiento tenga 22 empresas y el conjunto de prueba tenga 11 empresas), y se selecciona la mejor combinación de empresas cuyo error cuadrático en los promedios de costos de ventas sea menor. Esta tarea de la selección de las empresas fue realizada en un Notebook de python que se puede encontrar en el repositorio.
...
En este caso el modelo está siendo exclusivamente dependiente de la dinámica el PIB, y se tiene que un aumento en un 1% de la variación del PIB anual tiene un impacto positivo de hasta 12.70% sobre la variación en los costos de venta, lo cual es razonable, en particular porque al percibir mayor crecimiento en la economía global la industria, naturalmente, también se ve afectada al haber mayor producción y un mayor retorno de dinero; no obstante, las dinámicas de las compañías que componen la industria muchas veces se ven influenciada por el factor macro más global de manera diferentes, ello por su tamaño o tendencias, así que probablemente sea necesario modelar dichos efectos de una forma más robusta.
...
### Toca rotar la figura para poder ver el plano
fig %>% add_surface(z = ~z2, opacity = 0.98,colorscale = list(c(0,2),c("rgb(255,112,184)","rgb(120,0,64)"))) %>% layout(title="Poblacional (azul y verde) vs Un Individuo (Rosa)")
data_surface$NIT = empresas_train[20]
z3 = predict(mod_me, newdata = data_surface)
z3 = matrix(z3, nrow = 50, ncol = 50)

### Toca rotar la figura para poder ver el plano
fig %>% add_surface(z = ~z3, opacity = 0.98,colorscale = list(c(0,2),c("rgb(255,112,184)","rgb(220,100,164)"))) %>% layout(title="Poblacional (azul y verde) vs Un Individuo (Rosa)")
