...
- aquellas variables que indiquen variación porcentual se representarán en su forma absoluta, es decir, un valor de 7% se presenta como 0.07.
...
Para fines de intepretación de los modelos creados en el trabajo y para estabilizar un poco la varianza, consideraremos la transformación logarítmica tanto para las los predictores como para la variable respuesta para poder tener un modelo de tipo log-log, además de tener en cuenta que al trabajar con porcentajes, es posible que existan sesgos en la variable, así la transformación que se realiza es:
...
#### Boxplot para las variables sin log
...
En el grafico anterior, podemos ver la información de la distribución de las variables en niveles que se utilizan en el trabajo, vemos entonces que existen correlaciones fuertes entre las variables macro-económicas, (tal como nos lo habia indicado la matriz de correlación), pero adicional a esto, es claro que aquella variable que tiene una mayor variabilidad es la relacionada a la diferencia relativa de los costos de venta, pues observamos que para las variables macro-económicas sólo se tienen 3 barras en su histograma, lo cual se debe a que sólo contamos con la información de 3 años, la cual es la misma para todos ls individuos (empresas) que estamos considerando en este trabajo. 
...
```{r fig.align='center'}
p2 = grafica(all_data, "Costo.de.ventas_dif", "TRM")
```
...
Las gráficas anteriores, nos permiten ver el comportamiento de la distribución de dos variables, además de tener como referencia los contornos de una distribución normal bivariada. De estos gráficos, podemos concluir que aquellos pares de variables graficados, no son candidatas para afirmar que pueden seguir una distribución normal, pues claramente tenemos registros dentro del conjunto de datos que en particula tienen o valores bastante grandes en magnitud que caen por fuera de las regiones de normalidad. De igual manera, al considerar las dos variables macro-económicas incluidas (el PIB y la TRM), justo por la característica de las mismas en el conjunto de datos (tener sólo valores para 3 años), no nos dan información acerca de la dispersión de la variable, por el contrario, tiene un efecto en las gráficas de contorno, enfocando los datos en 3 lineas (una para cada año) y posteriormente incluir la variabilidad anual del logaritmo de la diferencia de los costos de venta. Nuevamente por la magnitud  de alguno de los valores en la variable explicativa, vemos que encontramos puntos de los indivuduos que caen por fuera de la región de normalidad, por lo tanto tampoco podemos concluir normlidad bivariada entre estas variables.
...
razonable en la industria en comparación al obtenido con el lineal generalizado, 
donde si bien hay unas compañías que se alejan del común respecto a las diferencias en los costos de venta, 
el resto permanece en escalas de entre -0.2% y 0.1% con una variedad muy interesante ya que
...
en el mercado. Respecto a las compañías que se hayan por fuera del intervalo se tienen
dos clases, por un lado aquellos cuyos valores reales son menores
a cero son compañías cuyo concepto de costos de ventas disminuyeron más de lo normal en el
período actual, y por otro lado (bajo la selección de compañías en entrenamiento)
donde los valores reales fueron considerablemente superiores a los estimados, lo que
refleja compañías cuyos costos de venta superaron la expectativa en el período actual.
...
* Debido a las características del conjunto de datos que estabamos utilizando (además de tener información limitada a 3 años), vemos que no es posible tener una buena capacidad predictiva en los modelos considerados, sin embargo, se puede apreciar que al realizar interpretación  de la estructura del modelo (inferencia), vemos que tiene sentido tanto que el PIB tenga una relación positiva como el Desempleo una relación negativa al momento de querer explicar la variación anual en los costos de venta de las empresas.

* Al tener poca información histórica (información de 3 años) pero contar con un número considerable de individuos (empresas) en el análsis, vemos que efectivamente el modelo más conveniente para ser utilizado es el modelo de efectos mixtos, pues vemos que podemos obtener un comportamiento promedio para la variación anual de la diferencia de costos de venta, pero es claro que el comportamiento en el mercado de cada una de las empresas es diferentes, por lo que al considerar los efectos aleatorios con el modelo de efecto mixto, obtenemos un modelo más realista al momento de hacer inferencia e interpretación, pues es posible tener diferentes aumentos anuales para las empresas dadas las mismas variables macro-económicas.
