...
```{r}
mod_me = lmer('Costo.de.ventas_dif~TRM+PIB+(TRM|NIT)+(PIB|NIT)', data = data_train_z)
summary(mod_me)
```
...
glm_obj <- glmmLasso(Costo.de.ventas_dif~TRM+PIB+Desempleo+Inflacion+Balance_CC+Balance_Fiscal+Tasa_Intervencion, rnd = list(NIT=~1+PIB), data = data_train_z, lambda=2.4, family = gaussian(link ="identity"))
summary(glm_obj)
```
...
Resultados = c(r2_score(preds, data_train_z$Costo.de.ventas_dif), adj_r2_score(preds, data_train_z$Costo.de.ventas_dif), mean((data_train_z$Costo.de.ventas_dif - preds)^2)) 
...
plot(preds,data_test_z$Costo.de.ventas_dif, main='Generalized linear model Lasso', sub=sub_tit)
abline(a=0, b=1, lwd = 2, col = 'red')
...
```{r, echo=FALSE}
Medidas = c("r2 train", "r2 adj train", "mse train") 
Resultados = c(r2_score(preds, data_train_z_sinoutl$Costo.de.ventas_dif), 
               -adj_r2_score(preds, data_train_z_sinoutl$Costo.de.ventas_dif), 
                mean((data_train_z_sinoutl$Costo.de.ventas_dif - preds)^2)) 
...
### Lambda bajo (variable PIB, TRM y Balance Fiscal)
```{r}
## Generalized linear model Lasso 
glm_obj_tr <- glmmLasso(Costo.de.ventas_dif~PIB+TRM+Balance_Fiscal, rnd = list(NIT=~1+PIB+TRM+Balance_Fiscal), data = df_trans, lambda=2.5, family = gaussian(link ="identity"))
summary(glm_obj_tr)
...
```{r, echo=FALSE}
Medidas = c("r2 train", "r2 adj train", "mse train") 
Resultados = c(r2_score(preds, data_train_z$Costo.de.ventas_dif), 
               -adj_r2_score(preds, data_train_z$Costo.de.ventas_dif), 
                mean((data_train_z$Costo.de.ventas_dif - preds)^2)) 
...
### Lambda alto  (variable PIB, TRM y Balance Fiscal)
```{r}
## Generalized linear model Lasso
