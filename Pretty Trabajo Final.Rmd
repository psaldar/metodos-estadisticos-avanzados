...
TRM = seq(min(data_train_z$TRM),max(data_train_z$TRM), length.out = 50)
PIB = seq(min(data_train_z$PIB),max(data_train_z$PIB), length.out = 50)

data_surface = expand.grid(TRM,PIB)
names(data_surface) = c('PIB','TRM')
...
fig <- plot_ly(x = PIB, y = TRM, z = z) %>% add_surface()
fig
