...
kable(
  ...
  kable_styling(bootstrap_options = "striped", full_width = F, position = "center", fixed_thead = T, font_size = 15)
```
...
```{r}
cov_data = cov(data)
kable(formatC(cov_data,format = "e", digits = 4)) %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "center", fixed_thead = T, font_size = 12)%>%
...
plot(preds, data_train_z$Costo.de.ventas_dif, xlab = 'Valor predicho', ylab = 'Valor Real', main='Modelo Lineal General')
...
```{r echo = FALSE}
Medidas = c("r2 train", "r2 adj train", "mse train") 
Resultados = c(r2_score(preds, data_train_z_noOut$Costo.de.ventas_dif), adj_r2_score(preds, data_train_z_noOut$Costo.de.ventas_dif), mean((data_train_z_noOut$Costo.de.ventas_dif - preds)^2)) 
