...
```{r}
par(mfrow=c(1,2))
boxplot(Costo_de_ventas~YEAR, data = all_data)
boxplot(Gastos_de_ventas~YEAR, data = all_data)
```
...
```{r}
mod_lin = lm('Costo_de_ventas~balance_fiscal+cuenta_corriente+desempleo+inflacion+pib+tasa_intervencion+trm', data = data_train)
```
...
plot(preds, data_train$Costo_de_ventas, xlab = 'Valor predicho', ylab = 'Valor Real')
abline(a=0, b=1, lwd = 2, col = 'red')
```

En el conjunto de Prueba
```{r}
preds_test = predict(mod_lin, newdata = data_test)
plot(preds_test, data_test$Costo_de_ventas, xlab = 'Valor predicho', ylab = 'Valor Real')
abline(a=0, b=1, lwd = 2, col = 'red')
```
...
X = model.matrix(f1, data = as.data.frame(data_train))
Y = data_train$Costo_de_ventas
...
```{r}
#mod_me = lmer(Costo_de_ventas~balance_fiscal+desempleo+inflacion+pib+tasa_intervencion+trm+(Costo_de_ventas|balance_fiscal)+(Costo_de_ventas|desempleo)+(Costo_de_ventas|inflacion)+(Costo_de_ventas|pib)+(Costo_de_ventas|tasa_intervencion)+(Costo_de_ventas|trm), data = data_train)
```
