...
kable(all_data[(all_data$Costo_de_ventas %in% bxplot_costos$out),]$NIT)
```

```{r}
### Outliers para gastos
kable(all_data[(all_data$Gastos_de_ventas %in% bxplot_gastos$out),]$NIT)
```
...
empresas = unique(all_data$NIT)

smp_size <- floor(0.75 * length(empresas) )

idx_train = sample(seq_len(length(empresas)), size = smp_size)

empresas_train = empresas[idx_train]
empresas_test = empresas[-idx_train]
...
De los gráficos anteriores, en particular analizando el valor de la distancia de Cook para las observaciones, notamos que las observaciones 15, 31, 48 y 64 son candidatas a ser registros atípicos en el conjunto de datos, por lo que entrenaremos un nuevo modelo eliminando estos registros. Notar que estos registros coinciden con las empresas con NIT 860009694 y 900378893 para los años 2016 y 2017, lo cual coincide con los registros raros encontrados en la sección descriptiva utilizando el boxplot

```{r}
### Vector para eliminar registros atipicos
outliers = c(15, 31, 48, 64)
...
```{r}
mod_lin02 = lm('Costo_de_ventas~pib+trm', data = data_train_z_noOut)
```
...
plot(preds, data_train_z_noOut$Costo_de_ventas, xlab = 'Valor predicho', ylab = 'Valor Real')
abline(a=0, b=1, lwd = 2, col = 'red')
```
...
f1 = formula('Costo_de_ventas~-1+balance_fiscal+desempleo+inflacion+pib+tasa_intervencion+trm')
...
#mod_me = lmer('Costo_de_ventas~Gastos_de_ventas+(Gastos_de_ventas|NIT)', data = data_train_z)
mod_me = lmer('Costo_de_ventas~pib+(pib|NIT)', data = data_train_z)
```
...
plot(preds,data_test_z$Costo_de_ventas)
abline(a=0, b=1, lwd = 2, col = 'red')
```
